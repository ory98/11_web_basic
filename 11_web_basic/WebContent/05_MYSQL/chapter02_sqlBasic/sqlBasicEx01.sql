/*
	...
	# 데이터 조작어 (DML : Data Manipulation Language) > INSERT문
	 ...
	 - 데이터베이스에 새로운 레코드를 추가한다. (세로 : 필드 , 컬럼 / 가로 : 로우 , 레코드)
...
/*
	...
	# 트랜잭션 제어 언어 (Transaction Control Language) : 상태의 변화
	...
	1) COMMIT : 트렌젝션을 데이터베이스에 반영한다.
		...
		- 기본적으로 autocommit 설정이 되어있는데 중요한 서버(운영)에는 autocommit 설정을 걸지 않는다.
